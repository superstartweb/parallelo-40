...
export default function Parallelo40Dashboard() {
  ...
  const fetchOrders = async () => {
    ...
    if (data) {
      const ordiniInArrivo = data.filter((o: any) => o.status === "in_consegna").length;
  ...
  const aggiornaStato = async (orderId: string, nuovoStato: string, phone: string | null, num: number, name: string) => {
    await supabase.from("orders").update({ status: nuovoStato }).eq("id", orderId);
    
    if (nuovoStato === "consegnato_chiosco" && phone) {
      const msg = `Ciao ${name}! Il tuo ordine N°${num} è pronto al Banco Parallelo40! 🍹 Vieni a ritirarlo!`;
    ...
    return (
      <div className="min-h-screen bg-[#00c0f4] flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-[40px] text-center max-w-md w-full shadow-2xl">
          ...
          <h1 className="text-2xl font-black text-gray-800 mb-8">Point Parallelo40</h1>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 pb-24">
      <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border-l-4 border-[#00c0f4]">
        <div>
          <h1 className="text-xl font-black text-gray-800">BANCO</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {orders.map((order) => (
          <div key={order.id} className={`bg-white rounded-3xl p-6 shadow-md border-t-8 ${order.status === 'in_consegna' ? 'border-orange-400' : 'border-green-500'}`}>
            ...
            <button 
              onClick={() => aggiornaStato(order.id, order.status === "in_consegna" ? "consegnato_chiosco" : "pagato", order.customer_phone, order.order_number, order.customer_name)}
